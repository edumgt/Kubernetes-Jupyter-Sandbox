...
│   └── k8s/
│       ├── base/        # 플랫폼 기본 워크로드
│       └── runner/      # GitLab Runner(k8s executor) 오버레이
...
1. [secret.example.yaml](/home/Kubernetes-OVA-SRE-Archi/infra/k8s/runner/secret.example.yaml#L1)의 `token` 값을 실제 runner token으로 바꿉니다.
2. 다음 명령으로 runner 리소스를 적용합니다.

```bash
bash scripts/apply_k8s.sh --with-runner
...
스크린샷과 로컬 확인에 사용한 기본 계정/토큰입니다. 실제 환경에서는 [platform-secrets.yaml](/home/Kubernetes-OVA-SRE-Archi/infra/k8s/base/platform-secrets.yaml#L1) 값을 반드시 변경해야 합니다.

- Airflow: `admin` / `admin12345!`
- JupyterLab token: `platform123`
- GitLab root: `v7Q#2mL!9xC@4pR%8tZ`
...
- 기본 k8s 매니페스트: [infra/k8s/base/kustomization.yaml](/home/Kubernetes-OVA-SRE-Archi/infra/k8s/base/kustomization.yaml#L1)
- Runner 오버레이: [infra/k8s/runner/kustomization.yaml](/home/Kubernetes-OVA-SRE-Archi/infra/k8s/runner/kustomization.yaml#L1)
