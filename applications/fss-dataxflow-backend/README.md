...
- 요청 본문(JSON): `{"username":"test1@test.com","password":"CHANGE_ME"}`
...
curl -sS "${API_BASE_URL}/api/auth/login" \
  ...
  -d '{"username":"test1@test.com","password":"CHANGE_ME"}'
```
...
TOKEN=$(curl -sS "${API_BASE_URL}/api/auth/login" \
  -H 'Content-Type: application/json' \
  -d '{"username":"admin@test.com","password":"CHANGE_ME"}' | jq -r '.access_token')
